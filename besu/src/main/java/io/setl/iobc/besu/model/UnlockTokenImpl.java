...
/**
 * Implementation of unlock tokens.
 *
 * @author Simon Greatrix on 19/11/2021.
 */
@Service
@Slf4j
public class UnlockTokenImpl implements UnlockToken {
  ...
  public CompletableFuture<TransactionResult> apply(ChainConfiguration configuration, Input input) throws ParameterisedException {
    ...
    log.info("Unlocking {} of token \"{}\" to \"{}\"", amount, tokenId, address);
    ...
    RemoteFunctionCall<TransactionReceipt> call = bnyToken.unlock(besuAdress, amount);
    TransactionReceipt receipt;
    try {
      receipt = call.send();
    } catch (Exception e) {
      // Failed to send mint request
      log.error("Failed to invoke unlock function on smart contract {} for token {} with amount {}",
