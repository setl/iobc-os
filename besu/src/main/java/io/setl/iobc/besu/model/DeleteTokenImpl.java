...
/**
 * Implementation of Delete Token.
 *
 * @author Simon Greatrix on 19/11/2021.
 */
@Service
@Slf4j
public class DeleteTokenImpl implements TerminateToken {

  private final AddressTable addressUtility;
  ...
  public CompletableFuture<TransactionResult> apply(ChainConfiguration configuration, Input input) throws ParameterisedException {
    ...
    SetlAddress setlAddress = addressUtility.getAddressSafe(token.getController());
    ...
    RemoteFunctionCall<TransactionReceipt> call = bnyToken.deleteToken();
    TransactionReceipt receipt;
    try {
      receipt = call.send();
    } catch (Exception e) {
      // Failed to send mint request
      log.error("Failed to delete smart contract {} for token {}",
